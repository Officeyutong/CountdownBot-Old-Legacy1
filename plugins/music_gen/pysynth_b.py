...
def make_wav(song, bpm=120, transpose=0, leg_stac=.9, boost=1.1, repeat=0, fn="out.wav", silent=False):
    ...
    for n in range(att_len):
        att_treb[n] = linint(((0, 0.), (100, .2), (300, .7), (400, .6),
                              (600, .25), (800, .9), (1000, 1.25), (2000, 1.15), (3000, 1.)), n)
        att_bass[n] = linint(((0, 0.), (100, .1), (300, .2), (400, .15),
                              (600, .1), (800, .9), (1000, 1.25), (2000, 1.15), (3000, 1.)), n)
    decay = np.zeros(1000)
    for n in range(900):
        decay[n] = exp(linint(((0, log(3)), (3, log(5)),
                               (5, log(1.)), (6, log(.8)), (9, log(.1))), n/100.))
    ...
    def render2(a, b, vol, pos, knum, note):
        ...
        raw_note = 12*44100
        snd_len = min(max(int(3.1*q), 44100), raw_note)
        fac = np.ones(snd_len)
        ...
        #     snd_len = raw_note
        # print(snd_len)
        snd_len = min(snd_len, raw_note)
        # print("snd_len=", snd_len)
        # print("len1=", len(data[pos:pos+snd_len]))
        # print("len2=", len(new[:snd_len]))
        right = (new[:snd_len] * fac * vol *
                 (1. + schweb_amp * np.sin(2. * pi * np.arange(snd_len)/schweb/32.)))
        # print("len right=", len(right))
        data[pos:pos+snd_len] += right
